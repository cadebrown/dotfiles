...
with tempfile.TemporaryDirectory(prefix="guard-test-") as temporary:
    ...
    runtime = root / "host-state" / "codex"
    for directory in (runtime / "agents", runtime / "rules", runtime / "themes"):
        directory.mkdir(parents=True, exist_ok=True)
    for relative in ("config.toml", "hooks.json", "AGENTS.md", "deep.config.toml",
                     "agents/coder.toml", "rules/dotfiles.rules", "themes/neon-noir.tmTheme",
                     "auth.json", "sessions/task.jsonl", "state_1.sqlite", "personal.config.toml"):
        target = runtime / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text("runtime\n")
    runtime_environment = {**environment, "CODEX_HOME": str(runtime), "DF_DOTFILES_REPO": str(ROOT)}
    ...
    # Host-local Codex config is generated by install/codex.sh, but its task
    # data/auth/database files remain native runtime state and must stay writable.
    for relative in ("config.toml", "hooks.json", "AGENTS.md", "deep.config.toml",
                     "agents/coder.toml", "rules/dotfiles.rules", "themes/neon-noir.tmTheme"):
        check({"file_path": str(runtime / relative)}, True, env=runtime_environment)
    for relative in ("auth.json", "sessions/task.jsonl", "state_1.sqlite", "personal.config.toml"):
        check({"file_path": str(runtime / relative)}, False, env=runtime_environment)
