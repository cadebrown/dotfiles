...
@test "Cursor CLI config is a write-once seed plus installer merge" {
    ...
    jq -e '.exploreSubagentModel == "composer-2.5"' \
        "$REPO_ROOT/home/dot_cursor/create_cli-config.json"
    jq -e '.selectedModel.modelId == "composer-2.5"' \
        "$REPO_ROOT/home/dot_cursor/create_cli-config.json"
    jq -e '.model.modelId == "composer-2.5"' \
        "$REPO_ROOT/home/dot_cursor/create_cli-config.json"
    jq -e '.hasChangedDefaultModel == true' \
        "$REPO_ROOT/home/dot_cursor/create_cli-config.json"
    grep -q '_sync_cursor_cli_config' "$REPO_ROOT/install/cursor.sh"
    grep -q 'exploreSubagentModel = $model' "$REPO_ROOT/install/cursor.sh"
    grep -q 'hasChangedDefaultModel = true' "$REPO_ROOT/install/cursor.sh"
    grep -q 'sync-cli' "$REPO_ROOT/install/cursor.sh"
}

@test "Cursor CLI merge preserves runtime keys and pins Composer 2.5" {
...
EOF
    run env HOME="$home" DF_USE_PLAT=0 bash -c '
        ...
        jq -e ".selectedModel.modelId == \"composer-2.5\"" "$HOME/.cursor/cli-config.json"
        jq -e ".selectedModel.parameters == [{\"id\":\"fast\",\"value\":\"false\"}]" "$HOME/.cursor/cli-config.json"
        jq -e ".model.modelId == \"composer-2.5\"" "$HOME/.cursor/cli-config.json"
        jq -e ".model.displayModelId == \"composer-2.5\"" "$HOME/.cursor/cli-config.json"
        jq -e ".model.extra == true" "$HOME/.cursor/cli-config.json"
        jq -e ".hasChangedDefaultModel == true" "$HOME/.cursor/cli-config.json"
...
@test "Cursor CLI merge replaces a null Auto default with Composer 2.5" {
