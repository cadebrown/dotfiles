...
- **Persistent account workspaces:** use `df-browser` for project/service profiles, private auth transfers, login observations, screenshots and traces. Read `~/dotfiles/docs/usage/browser-sessions.md` for commands. `open` creates an owned profile; explicit `attach --extension` retains native browser consent, and `close` detaches external browsers. Reuse the named workspace across sessions without exporting auth state by default.
