...
_SERVICE_DEFS=(
    ...
    "asta|ASTA_API_KEY|.asta.env|Ai2 Asta scholarly-corpus MCP key (200M+ papers, snippet search, citation graph — free)|https://allenai.org/asta/resources/mcp|—|you don't use the Asta scholarly MCP"
    "aristotle|ARISTOTLE_API_KEY|.aristotle.env|Harmonic Aristotle formal-prover API (aristotlelib; Lean proofs as a service)|https://aristotle.harmonic.fun/|—|you don't use the Aristotle prover API"
    "aleph|ALEPH_API_KEY|.aleph.env|Aleph Prover API (Logical Intelligence; pip alephprover — agentic Lean 4 prover)|https://logicalintelligence.com/|—|you don't use the Aleph prover API"
    ...
    "resend|RESEND_API_KEY|.resend.env|Resend API key (transactional mail from ~/dev/nollege)|https://resend.com/api-keys|sending access|you don't run a project that sends mail"
    "elevenlabs|ELEVENLABS_API_KEY|.elevenlabs.env|ElevenLabs API key (text-to-speech narration in ~/dev/kardinality)|https://elevenlabs.io/app/developers/api-keys|keys are restricted by default — enable text_to_speech|you don't render narrated video"
    "deepgram|DEEPGRAM_API_KEY|.deepgram.env|Deepgram API key (speech-to-text / voice agent; a hosted alternative to the Cloudflare Workers AI whisper binding in ~/dev/nollege)|https://console.deepgram.com|a least-privilege role that can call the API — keys:write is only for managing other keys|you transcribe via the Workers AI whisper binding (no key needed)"
    ...
    "gitlab-host|GITLAB_HOST|.gitlab.env|GitLab instance this machine talks to — glab's own variable, so a self-managed host needs no extra flags (leave empty for gitlab.com)|https://gitlab.com|—|this machine uses gitlab.com, which is glab's default"
...
case "$_mode" in
    ...
    gh|github-cli)
        ...
        ;;
    google)
        # `auth.sh google [client_secret.json]` — the file arg is consumed here
        # (not as a service in the *) loop) so the Workspace tier can use your
        # own OAuth client.
