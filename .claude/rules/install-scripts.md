...
| `lean.sh` | elan (Lean's rustup) → `$ELAN_HOME` + the pinned default toolchain | `DF_LEAN_TOOLCHAIN` overrides the pin; bump it deliberately alongside Mathlib releases (4.32.1 fixed a kernel soundness bug, so stale toolchains are a correctness risk). Upgrade mode runs `elan self update` only — `elan update` is deliberately not called, since every toolchain here is an exact pin. Mathlib is cache-first (`lake exe cache get`); source builds want 16 GB+ RAM. |
| `latex.sh` | macOS: verify the MacTeX cask. Linux: TinyTeX + base packages | Linux has no sudo, so TinyTeX (user-prefix TeX Live, ~200 MB) fills the hole; `tlmgr option sys_bin "$ARCH_BIN"` keeps binaries in the PLAT layout instead of TinyTeX's `~/.local/bin` default. macOS puts nothing in a PATH dir — the shell profiles append `/Library/TeX/texbin`. Upgrade mode runs `tlmgr update --self --all`; on macOS that needs a sudo ticket and is skipped without one. |
| `claude.sh` | Claude Code binary + plugins + MCP servers + overlay skills + `~/AGENTS.md` symlink | Downloads from Anthropic's release CDN (`downloads.claude.ai`; native installer deliberately bypassed — no path-relocation knob for PLAT/NFS); overlay discovery via `DF_OVERLAYS`. MCP servers reconcile declaratively (URL/command drift re-registers). Auth details: see `.claude/rules/agent-tooling.md`. |
...
| `linearmouse.sh` | Tracks LinearMouse (macOS GUI) settings. `apply` (default) merges tracked settings into the app-owned config; `sync` captures in-app changes back | macOS only (self-skips on Linux). App via Brewfile (`cask "linearmouse"`). NOT chezmoi-managed, and the reason is specific: LinearMouse stamps `"$schema": ".../<app version>"` into the live file and rewrites it on every app update, so a managed copy goes dirty **on a timer** — no setting has to change. The tracked source at `install/linearmouse/linearmouse.json` deliberately omits `$schema`; `apply` merges live-first so the app's version stamp survives. `schemes` is an array, so the merge replaces rather than appends — that's the only way to delete a per-device rule from the repo. |
| `cursor.sh` | Cursor MCP, settings links, extension install, CLI-config merge (Composer 2.5 default), My Machines worker; `check` / `sync-mcp` / `sync-cli` / `sync-worker` / `sync-extensions` | Union-only extensions (never removes); app via Brewfile cask. `cli-config.json` is create_+merge. Worker is Darwin-only; TCC is a human grant. |
...
| `local-llm.sh` | Verifies ollama/mlx-lm/mlx-openai-server binaries; creates HF cache dir; `pull-models` subcommand pre-pulls MLX models from `packages/mlx-models.txt` | Fails when selected tools are missing or unhealthy. MLX is the primary local backend (started via `mlxserve`); Ollama remains as fallback. |
...
| `blender-mcp.sh` | Installs the `blender-mcp` Blender addon (`addon.py` from github.com/ahujasid/blender-mcp) and enables it via headless Blender | MCP server side is separate — see `packages/mcp-servers.txt`. Fails if selected and Blender is not installed. |
...
6.   runtimes         DF_DO_PYTHON, DF_DO_NODE, DF_DO_RUST, DF_DO_GO, DF_DO_LEAN, DF_DO_LATEX, DF_DO_CLAUDE, DF_DO_CODEX, DF_DO_CLAUDE_DESKTOP, DF_DO_CODEX_DESKTOP, DF_DO_CURSOR, DF_DO_VSCODE, DF_DO_CMAKE
...
6.7  blender-mcp      DF_DO_BLENDER_MCP (fails if selected and Blender is absent)
