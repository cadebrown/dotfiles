...
| `cursor.sh` | Cursor MCP, settings links, extension install, CLI-config merge, My Machines worker; `check` / `sync-mcp` / `sync-cli` / `sync-worker` / `sync-extensions` | Union-only extensions (never removes); app via Brewfile cask. `cli-config.json` is create_+merge. Worker is Darwin-only; TCC is a human grant. |
